...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_password == $confirm_password) {
        ...
        $query = "SELECT * FROM signup WHERE password1 = '$CurrentPassword'";
        $result = $connect->query($query);

        if ($result->num_rows > 0) {
            // Update the password if the current password matches
            $update_query = "UPDATE signup SET password1='$new_password', password2='$new_password' WHERE password1 = '$CurrentPassword'";
